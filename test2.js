...
export const realtedProductController = async (req, res) => {
  try {
    ...
    const trained_data = JSON.parse(fs.readFileSync(${__dirname}/trained_data.json, 'utf8'));
    // console.log(trained_data)
    const r_id = trained_data[pid]
    const product_id = []  
    r_id.map(dict => {
        product_id.push(dict['id'])
    });
    // console.log(product_id);
    const products = await productModel
    .find({ _id: { $in: product_id } })
    .select("-photo")
    .limit(3)
    .populate("category");
